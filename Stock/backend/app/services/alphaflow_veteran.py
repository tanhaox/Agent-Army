...
async def backtest_veteran_breakout_rate(lookback_days: int = 180) -> dict:
    ...
    async with async_session_factory() as s:
        ...
        r = await s.execute(text("""
            SELECT ap.ts_code, ap.scan_date, ap.veteran_level, ap.veteran_score,
                   ap.in_pool, ap.xgb_prob,
                   dk.close as scan_close,
                   dk5.close as close_5d, dk20.close as close_20d
            FROM alphaflow_pool ap
            JOIN daily_kline dk ON dk.ts_code = ap.ts_code AND dk.trade_date = ap.scan_date
            LEFT JOIN daily_kline dk5 ON dk5.ts_code = ap.ts_code
                AND dk5.trade_date = (SELECT trade_date FROM daily_kline
                    WHERE ts_code = ap.ts_code AND trade_date > ap.scan_date
            ...
            WHERE ap.scan_date >= :cut
              AND ap.veteran_detected = true
            ORDER BY ap.scan_date DESC
        """), {"cut": cutoff})
        rows = r.fetchall()
    ...
    for row in rows:
        ts_code, scan_date, level, vet_score, in_pool, xgb_prob = row[0:6]
        scan_close = float(row[6] or 0)
        close_5d = float(row[7] or 0)
        close_20d = float(row[8] or 0)
