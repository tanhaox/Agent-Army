...
LINES = [
    ...
    "echo [1/5] 正在清除旧进程...",
    "",
    'for %%p in (8002 8003 8000 5173 5174) do (',
    '    for /f "tokens=5" %%a in (\'netstat -ano ^| findstr ":%%p.*LISTENING" 2^>nul\') do (',
    "        echo   终止 PID %%a (占用端口 %%p)",
    "        taskkill /F /PID %%a 1>nul 2>&1",
    "    )",
    ")",
    "",
    'for /f "tokens=1" %%a in (\'wmic process where "name=\'python.exe\'" get processid 2^>nul ^| findstr /r "[0-9]"\' ) do (',
    '    wmic process where "processid=%%a" get commandline 2^>nul | findstr "uvicorn app.main" >nul && (',
    "        echo   终止 uvicorn 进程 PID %%a",
    "        taskkill /F /PID %%a 1>nul 2>&1",
    "    )",
    ")",
    "",
    'taskkill /F /IM celery.exe 1>nul 2>&1',
