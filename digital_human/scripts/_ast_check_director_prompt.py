"""AST 硬约束检查 — director_prompt 包.

对每个模块: 文件 ≤250 行; 函数 ≤40 行 (--orch 名单内的编排函数 ≤70);
类 ≤200 行; 模块 ≤3 个类; 无 `from xxx import *`; __init__.py 含 __all__。

用法: .venv/Scripts/python.exe scripts/_ast_check_director_prompt.py
      --orch build_director_prompt ...   (声明编排函数, 上限放宽到 70)
"""
import argparse
import ast
import pathlib
...
PKG = pathlib.Path("app/services/director_prompt")
MAX_FILE = 250
MAX_FUNC = 40
MAX_ORCH = 70
MAX_CLASS = 200
