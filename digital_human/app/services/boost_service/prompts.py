...
拆包自 boost_service.py , 提示词原文一字不差.
"""
...
要求：不直接问问题，通过"反讽"或"质疑"视角，给观众"想反驳/想选边"的切入点。
**时序结构 **：前段埋**闭合性二选一**钩子（"打嘴炮还是真能降"式）——低流失位代价小，观众带着自己的选择往下看（想验证→完播），并为结尾回收埋线。
...
# 📐 结构分布协议（禁止结尾拥堵）
1. 【争议回收】→ 放置在 [价值观收割段] 的末尾。**时序结构二选一 **：
   a. **开放性问句**（首选）——观众凭自身经历/利益作答，人人有话说（例："美联储真降了，你觉得咱们的利息还能跟着降吗"）——听完的人有表达欲，畅所欲言；
   b. **回收前段闭合**——"开头问过大家：这次他是打嘴炮还是真能降？听完了，你怎么选？"——只回收已埋的线，**禁止凭空新抛闭合问句**（观众心里答完就走不落评论）。
   禁止提及"下期见"或"关注"。
2. 【关注回收】→ 放置在 [固定结尾] 之前，仅限一句话。基于信息钩子给出必须关注的具体理由。禁止长篇价值观输出。
3. 【固定结尾】→ 必须 1:1 原样保留：听懂逻辑，少走弯路。关注我，别人看热闹，你听门道。||我是{persona}，||下期见。
...
听懂逻辑，少走弯路。关注我，别人看热闹，你听门道。||我是{persona}，||下期见。  ← 固定结尾
```
...
- **金句去重 **：全稿同一句金句只出现一次 — 收尾定调金句禁止与中段已用过的重复，中段和结尾各给不同的金句。
...
- **字数下限锁**：输出总字数不得低于原文的 95%。精修 = 打磨表达，不是压缩——禁止把多个短句合并成长句来省字数。若精修后字数不足，用强化画面感的修饰补足，禁止注水复述。
...
- **型号真实形**：型号/版本号原样保留（GLM-5.3、GPT-6、o3-mini）— 读法由系统 TTS 层自动转换；禁手写改形（GPT六/GLM五点三 = 字幕一眼假）。
- **清除预告/计数句**：「这篇的赞，我给三条」「下面说三件事」等内容预告/计数句直接删除，只保留内容本身。
...
# 标注规则
- 分 8~14 个区间，区间连续不重叠，合起来覆盖全部句子；区间边界只在语义模块切换处（背景→硬事实→人物→对照→升华）。
- **全程惊讶打底（主基调铁律, 2026-08-27 用户定稿）**：所有区间默认情绪一律 surprised。起伏不靠换情绪，靠强度档坡度——叙述/铺垫 2-3 档，硬事实/数据爆点 4-6 档，全篇至多一个 7 档；相邻区间强度差 ≤2。
- **波浪铁律**：强度必须有潮汐——爬坡逐级上、峰顶短、回落喘气再爬下一座山。≥5 档的区间 ≤8 句；全篇 ≥5 档句子合计 ≤ 全篇 1/3（大部分句子住在 2-4 档）；7 档峰顶区间 ≤3 句且其后必须回落到 ≤4 档。禁"高压平台"：连续多个高档区间首尾相接视为一个超长爆点，同样受 ≤8 句约束。
...
# P5 span 协议·老谭读书版 (2026-09-07 双人物立): confident+serious 打底 —
# 毒舌拆书人设, "牌桌边复盘"的笃定与锋利; 与静读书 calm 治愈调差异化
# (用户 0907 拍板: 先这么设定, 成品后听效果再定)。
P5_SPAN_PROMPT_BOOK_LAOTAN = """你是{persona}的【情绪标注师】。下面是编号好的老谭读书口播句子表。你的唯一任务：把句子分组连续区间，给每个区间标「情绪 + 强度档」。不评价、不改写、不解释。

# 情绪枚举（只能从中选, 英文小写）
calm / confident / serious / surprised / happy / melancholic

# 标注规则
- 分 6~10 个区间，区间连续不重叠，合起来覆盖全部句子；区间边界只在语义模块切换处（钩子→引入→概念拆解→案例复盘→落到读者→收尾）。
- **confident+serious 混合打底（老谭读书基调, 2026-09-07 定稿）**：转述书中情节/背景/过渡句默认 confident（3-4 档，"过来人讲给你"的笃定松弛）；进入拆穿/点破/毒舌评价的区间切 serious（3-5 档，锋利、有分量但不吼）。
- serious 也可用于郑重核心提醒（全篇 ≤3 个区间）。
- surprised 仅用于书中反常识爆点（4-5 档），全篇 ≤2 个区间。
- calm 仅用于节奏放缓的过渡/共情段（2-3 档，全篇 ≤2 个区间）——老谭也有体谅，但不多话。
- happy 仅用于结尾收获/升华区间（3-4 档）。
- melancholic 仅用于书中人物困境的体恤（2 档，≤1 个区间）——陪一句就走，不沉溺。
- 禁止 angry / sad / afraid / disgusted——锋利靠内容不靠吼；相邻区间强度差 ≤2。

# 输出（严格 JSON, 不要其他文字）
{"spans": [[起始句号, 结束句号, "情绪", 强度], ...]}
例: {"spans": [[1, 3, "confident", 3], [4, 10, "confident", 4], [11, 18, "serious", 4], [19, 24, "confident", 3], [25, 30, "happy", 4]]}"""
...
- **评论**: 是否预埋冲突点（官方vs现实/官方内部两套说辞打架/滤镜vs真相）、**互动时序结构**（前段闭合二选一埋线拉完播 + 尾段开放性问句畅言 / 或回收开头闭合"你怎么选"——禁凭空新抛闭合问句, 2026-09-06 用户技巧）、主动引导、跨界延伸（职场/管理/认知拓宽评论人群）
...
- **问的顺序 = 后文回收的顺序**：第 1 问必须是正文最早回收的模块（通常是背景/硬事实），横向对照类（历史对比）放最后一问——问序与后文错位 = 悬念悬而不收，观众等不到答案就划走。
